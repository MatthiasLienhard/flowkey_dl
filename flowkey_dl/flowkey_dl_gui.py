...
class MainWindow(object):
    def __init__(self, master):
        ...
        self.scale = Scale(layout, from_=10, to=200,
        ...
        self.space = Scale(layout, from_=10, to=200,
                           orient=HORIZONTAL, command=self.update_preview)
        self.space.set(50)
        ...
        self.apply_b = Button(measure, text="apply",
        ...
        self.prev_scale = Scale(self.prev_scale_frame, from_=10, to=100,
                                orient=HORIZONTAL, command=lambda x=None: self.update_preview(2000))
        self.prev_scale.set(25)
        self.prev_scale.grid(row=0, column=1)
        self.image = None
        self.current_page = 0
        self.canvas = Canvas(preview, width=self.prev_width,
                             height=self.prev_height, bg='white')  # 100 dpi
        self.prev_img = [ImageTk.PhotoImage(i) for i in arange_image(
            width=self.prev_width, height=self.prev_height)]
    ...
    def _get_preview(self):
        ...
        self.prev_img = [ImageTk.PhotoImage(
            i.resize((self.prev_width, self.prev_height))) for i in self.processed_images]
