...
def flowkey_dl(url):
    ...
    while True:
        ...
        if len(patch.shape) == 3:
            ...
            print(
                f'patch {i} looks strange, ignoring: {patch} \nshape: {patch.shape}')
        i += 1
...
def arange_image(image=None, title='', author='', width=2480, height=3508, scale=1, space=50, sel_measures=None, break_measures=None, nobreak_measures=None, font_size=(40, 20), mar=50):
    sel_measures, break_measures, nobreak_measures = [parse_nums(
        val) for val in (sel_measures, break_measures, nobreak_measures)]
    ...
    if sel_measures:
        ...
        image = np.hstack([image[:, measures[m-1]:measures[m]]
                          for m in sel_measures if m < len(measures)])
        # offset=0
        rm = [0]
        # new_measures=list()
